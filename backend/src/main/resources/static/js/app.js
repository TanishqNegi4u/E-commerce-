...
// ✅ In-memory auth (no localStorage — Render safe)
// ✅ Backend cart sync  ✅ Checkout modal with address
// ✅ Payment options (COD / UPI / Card)  ✅ Live product fetch
...
// ── APP STATE (in-memory only — no localStorage) ─────────────
let cart        = [];
let wishlist    = [];
let _token      = null;
...
const setAuth   = (token, user) => { _token = token; currentUser = user; };
const clearAuth = ()            => { _token = null;  currentUser = null; };
...
function goToSlide(i) { currentSlide = i; changeSlide(0); }
...
// ============================================================
// CART  (in-memory + backend sync when logged in)
// ============================================================
...
function addToCart(id, qty = 1) {
  ...
  renderCartPanel();
  if (getToken()) syncCartItemToBackend(id, existing ? existing.qty : qty);
}
...
async function syncFullCartToBackend() {
  if (!getToken() || !cart.length) return;
  try {
    await apiFetch('/cart', {method: 'DELETE'});
    for (const item of cart) await syncCartItemToBackend(item.id, item.qty);
...
function removeFromCart(id) {
  ...
  if (getToken()) apiFetch(`/cart/items/${id}`, {method: 'DELETE'}).catch(() => {});
}
...
function getCartTotals() {
  ...
  const couponOff = appliedCoupon ? Math.round(subtotal * appliedCoupon.pct / 100) : 0;
  const total     = subtotal + shipping - discount - couponOff;
  return {subtotal, shipping, discount, couponOff, total};
}
...
function renderCartPanel() {
  ...
  container.innerHTML = cart.map(item => {
    ...
    return `
      <div class="cart-item">
        <div class="item-image">${p.imageUrl
          ? `<img src="${p.imageUrl}" style="width:60px;height:60px;object-fit:contain" onerror="this.outerHTML='${p.image || '📦'}'">`
          : p.image || '📦'}</div>
  ...
  set('cartSubtotal', '₹' + subtotal.toLocaleString());
  set('cartShipping', shipping === 0 ? 'FREE' : '₹' + shipping);
  set('cartDiscount', '-₹' + discount.toLocaleString());
  set('cartTotal',    '₹' + total.toLocaleString());
}
...
const COUPONS = {
  ...
  'WELCOME': {pct: 15, label: '15% Welcome Offer'},
  'FLASH20': {pct: 20, label: '20% Flash Sale'}
};
...
function applyCoupon() {
  ...
  if (COUPONS[code]) {
    ...
    if (msgEl) { msgEl.textContent = `✅ "${code}" applied — ${appliedCoupon.label}`; msgEl.style.color = 'green'; }
    showToast(`Coupon ${code} applied! 🎉`, 'success');
    renderCheckoutSummary();
  } else {
    appliedCoupon = null;
    if (msgEl) { msgEl.textContent = '❌ Invalid coupon code'; msgEl.style.color = 'red'; }
  }
}
...
function openCheckoutModal() {
  ...
  if (!getToken())   { openAuthModal(); showToast('Please login to checkout', 'info'); return; }
...
function selectPayment(method) {
  ['COD','UPI','CARD'].forEach(m => {
    ...
    if (!el) return;
    el.style.border     = m === method ? '2px solid #7c3aed' : '2px solid #ddd';
    el.style.background = m === method ? '#f5f3ff' : '';
  });
...
function renderCheckoutSummary() {
  const {subtotal, shipping, discount, couponOff, total} = getCartTotals();
  const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
  set('ckSubtotal', '₹' + subtotal.toLocaleString());
  set('ckShipping', shipping === 0 ? 'FREE' : '₹' + shipping);
  set('ckDiscount', '-₹' + discount.toLocaleString());
  set('ckCoupon',   appliedCoupon ? '-₹' + couponOff.toLocaleString() : '—');
  set('ckTotal',    '₹' + total.toLocaleString());
}
    ...
    return `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #f0f0f0">
        <div style="display:flex;align-items:center;gap:10px">
          ...
          <div>
            <div style="font-size:13px;font-weight:600;max-width:180px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${p.name}</div>
  ...
  document.getElementById('checkoutContent').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px">
      ...
      <div>
        ...
        <div style="display:flex;flex-direction:column;gap:10px">
          <label id="pay-COD" onclick="selectPayment('COD')" style="display:flex;align-items:center;gap:12px;padding:14px;border:2px solid #7c3aed;border-radius:10px;cursor:pointer;background:#f5f3ff">
            <input type="radio" name="payment" value="COD" checked style="accent-color:#7c3aed">
            <span style="font-size:20px">💵</span>
            <div><div style="font-weight:700;font-size:14px">Cash on Delivery</div><div style="font-size:12px;color:#888">Pay when your order arrives</div></div>
          </label>
          <label id="pay-UPI" onclick="selectPayment('UPI')" style="display:flex;align-items:center;gap:12px;padding:14px;border:2px solid #ddd;border-radius:10px;cursor:pointer">
            <input type="radio" name="payment" value="UPI" style="accent-color:#7c3aed">
            <span style="font-size:20px">📲</span>
            <div><div style="font-weight:700;font-size:14px">UPI / PhonePe / GPay</div><div style="font-size:12px;color:#888">Pay instantly via UPI</div></div>
          </label>
          <label id="pay-CARD" onclick="selectPayment('CARD')" style="display:flex;align-items:center;gap:12px;padding:14px;border:2px solid #ddd;border-radius:10px;cursor:pointer">
            <input type="radio" name="payment" value="CARD" style="accent-color:#7c3aed">
            <span style="font-size:20px">💳</span>
            <div><div style="font-weight:700;font-size:14px">Credit / Debit Card</div><div style="font-size:12px;color:#888">Visa, Mastercard, RuPay</div></div>
          </label>
        </div>
      </div>
      ...
      <div>
        ...
        <div style="display:flex;gap:8px;margin-bottom:16px">
          <input id="couponInput" placeholder="Coupon code (e.g. SAVE10)" style="flex:1;padding:9px 12px;border:1.5px solid #ddd;border-radius:8px;font-size:13px;outline:none">
          <button onclick="applyCoupon()" style="padding:9px 16px;background:#7c3aed;color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:13px;font-weight:700">Apply</button>
        </div>
        <div id="couponMsg" style="font-size:12px;margin-bottom:12px"></div>

        <!-- Price breakdown -->
        <div style="background:#f9f9f9;border-radius:10px;padding:16px;font-size:14px">
          <div style="display:flex;justify-content:space-between;padding:5px 0"><span>Subtotal</span><span id="ckSubtotal">—</span></div>
          <div style="display:flex;justify-content:space-between;padding:5px 0"><span>Shipping</span><span id="ckShipping">—</span></div>
          <div style="display:flex;justify-content:space-between;padding:5px 0;color:green"><span>Loyalty Discount (5%)</span><span id="ckDiscount">—</span></div>
          <div style="display:flex;justify-content:space-between;padding:5px 0;color:green"><span>Coupon</span><span id="ckCoupon">—</span></div>
          <hr style="margin:10px 0;border-color:#eee">
          <div style="display:flex;justify-content:space-between;font-weight:800;font-size:16px"><span>Total</span><span id="ckTotal" style="color:#7c3aed">—</span></div>
        </div>

        <button onclick="placeOrder()" style="width:100%;margin-top:20px;padding:15px;background:linear-gradient(135deg,#7c3aed,#a855f7);color:#fff;border:none;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer;letter-spacing:0.5px">
          <i class="fas fa-check-circle"></i> Place Order
        </button>
        <p style="text-align:center;font-size:11px;color:#aaa;margin-top:10px">🔒 Secured by ShopWave</p>
      </div>
    </div>`;
...
async function placeOrder() {
  ...
  const btn = document.querySelector('#checkoutModal button[onclick="placeOrder()"]');
...
function showToast(msg, type = 'success') {
  ...
  toast.style.background = type === 'error' ? '#e74c3c' : type === 'info' ? '#2196F3' : '';
...
function updateNavAuth() {
  ...
  if (btn) btn.textContent = currentUser ? (currentUser.name?.split(' ')[0] || 'Account') : 'Login';
}
...
async function submitAuth() {
  ...
  try {
    if (authTab === 'login') {
      ...
      setAuth(data.accessToken, {id: data.userId, email: data.email, name: data.firstName, role: data.role});
...
async function showOrders() {
  if (!getToken()) { openAuthModal(); showToast('Please login to view orders', 'info'); return; }
...
function statusColor(status) {
  return {PENDING:'#FF9800', CONFIRMED:'#2196F3', SHIPPED:'#00BCD4',
          DELIVERED:'#4CAF50', CANCELLED:'#F44336', REFUNDED:'#607D8B'}[status] || 'var(--primary)';
}
...
const starsHtml = (rating, max = 5) => {
  ...
  return Array.from({length: max}, (_, i) =>
    `<span class="star ${i + 1 <= r ? '' : 'empty'}">${i + 1 <= r ? '★' : '☆'}</span>`
  ).join('');
};
